...
/* ---------------------------------------------------------
   Free rule-based FAQ knowledge base — no API calls, $0 cost.
   Matches user messages against keyword sets. Add more entries
   any time by copying the pattern below.
--------------------------------------------------------- */
const FAQ_KB = [
  {
    keywords: ["sleep", "shelter", "bed", "tonight", "stay", "nowhere to go"],
    response: "For a place to sleep tonight, check the Find Resources tab and filter by Shelter. Most emergency shelters accept walk-ins in the evening, though some fill up — calling ahead helps. If none have space, ask staff about overflow shelter locations, which many cities open on short notice.",
  },
  {
    keywords: ["food", "hungry", "eat", "meal", "groceries"],
    response: "For food today, check the Find Resources tab and filter by Food — food banks and soup kitchens are listed there with hours. Most don't require ID for a first visit.",
  },
  {
    keywords: ["shower", "laundry", "wash", "clean", "hygiene"],
    response: "Filter Find Resources by Shower & Laundry. Day centers often offer free showers, towels, and hygiene kits, and some community programs offer free wash-and-dry once a week.",
  },
  {
    keywords: ["doctor", "sick", "medical", "clinic", "health", "hospital", "hurt", "pain"],
    response: "Filter Find Resources by Medical & Mental Health. Walk-in clinics listed there see people regardless of insurance status. If this is a medical emergency, please use the Call 911 button at the top of the screen instead.",
  },
  {
    keywords: ["mental health", "depressed", "anxious", "counseling", "therapist", "therapy"],
    response: "Filter Find Resources by Medical & Mental Health for same-day counseling intake options. If you're in crisis right now, please use the 988 Crisis Lifeline button at the top of this screen — it's free and available right away.",
  },
  {
    keywords: ["id", "identification", "license", "birth certificate", "social security card", "documents", "paperwork"],
    response: "Check the Guides tab and open \"Replacing lost documents\" — it walks through getting a state ID, birth certificate, and Social Security card replaced, including fee-waiver options for people experiencing homelessness.",
  },
  {
    keywords: ["snap", "medicaid", "benefits", "ssi", "ssdi", "tanf", "food stamps", "assistance"],
    response: "Open the Guides tab and check \"Applying for benefits\" — it covers SNAP, Medicaid, SSI/SSDI, and TANF, including what to do if you don't have all your documents yet.",
  },
  {
    keywords: ["veteran", "va", "military", "service member"],
    response: "Filter Find Resources by Veterans for your local VA outreach office — they handle benefits screening and veteran-specific housing referrals.",
  },
  {
    keywords: ["job", "work", "employment", "resume", "interview", "hire"],
    response: "Open the Guides tab and check \"Finding work,\" or filter Find Resources by Employment for local workforce centers offering free resume help and same-day day-labor postings.",
  },
  {
    keywords: ["bus", "transit", "transportation", "ride", "get around"],
    response: "Filter Find Resources by Transportation for reduced-fare and transit voucher programs in your area.",
  },
  {
    keywords: ["family", "kids", "children", "baby"],
    response: "Filter Find Resources by Shelter — several listed programs specifically accept families with children. Case managers there can also connect you with family-specific benefits and school-enrollment help.",
  },
  {
    keywords: ["clothes", "clothing", "coat", "shoes"],
    response: "Filter Find Resources by Clothing for free clothing closets, including interview-appropriate attire.",
  },
  {
    keywords: ["hello", "hi", "hey", "help"],
    response: "I'm here to help. Tell me what you need most right now — sleep, food, medical care, benefits, or something else — and I'll point you to the right resource.",
  },
];

const FAQ_FALLBACK =
  "I don't have a specific match for that yet, but the Find Resources tab lets you browse by category, and the Guides tab covers benefits, documents, and employment. If this is urgent, use the emergency bar at the top of the screen.";
...
/* ---------------------------------------------------------
   AI Assistant Panel — free rule-based FAQ matching, no API
   calls and no cost. Swap matchFAQ() for the /api/chat call
   (see the commented block below) any time you're ready to
   pay for real AI responses.
--------------------------------------------------------- */
function AIAssistantPanel() {
  ...
  const send = useCallback(() => {
    ...
    // Small delay so it still feels conversational, not instant/robotic.
    setTimeout(() => {
    ...
    /* ------------------------------------------------------
       To switch to real AI responses later (once billing is
       set up in the Anthropic Console), replace the setTimeout
       ...
       fetch("/api/chat", {
         ...
         .then((res) => res.json())
         .then((data) => setMessages((prev) => [...prev, { role: "assistant", text: data.reply }]))
         .catch(() => setMessages((prev) => [...prev, { role: "assistant", text: "Something went wrong reaching the assistant." }]))
